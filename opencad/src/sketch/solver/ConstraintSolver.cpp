...
int ConstraintSolver::totalDOF(const std::vector<SketchEntity::Ptr>& entities) const {
    int dof = 0;
...
int ConstraintSolver::constrainedDOF(const std::vector<Constraint::Ptr>& constraints) const {
    int dof = 0;
...
int ConstraintSolver::remainingDOF(const std::vector<SketchEntity::Ptr>& entities,
                                    const std::vector<Constraint::Ptr>& constraints) const {
    return totalDOF(entities) - constrainedDOF(constraints);
}

bool ConstraintSolver::isFullyConstrained(const std::vector<SketchEntity::Ptr>& entities,
                                           const std::vector<Constraint::Ptr>& constraints) const {
...
Eigen::VectorXd ConstraintSolver::buildErrorVector(
    ...
    Eigen::VectorXd errors(numConstraints);
    int idx = 0;
    for (const auto& c : constraints) {
        if (c->isEnabled() && c->isDriving()) {
            errors(idx++) = c->error();
...
Eigen::MatrixXd ConstraintSolver::buildJacobian(
    ...
    Eigen::MatrixXd J = Eigen::MatrixXd::Zero(numConstraints, numParams);
    
    int constraintIdx = 0;
    for (const auto& constraint : constraints) {
        if (!constraint->isEnabled() || !constraint->isDriving()) continue;
        ...
        for (const auto& entity : constraintEntities) {
            ...
            for (int i = 0; i < entity->parameterCount() && jacIdx < static_cast<int>(jac.size()); ++i) {
                J(constraintIdx, paramOffset + i) = jac[jacIdx++];
            }
        }
...
bool ConstraintSolver::newtonStep(
    ...
    // Solve J * delta = -errors using pseudo-inverse (for over/under-constrained systems)
...
SolverStatus ConstraintSolver::solve(
    std::vector<SketchEntity::Ptr>& entities,
    const std::vector<Constraint::Ptr>& constraints) {
